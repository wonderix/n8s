...
type
  WebhookConversion* = object
    `clientConfig`*: WebhookClientConfig
    `conversionReviewVersions`*: seq[string]

proc load*(self: var WebhookConversion, parser: var JsonParser) =
  if parser.kind != jsonObjectStart: raiseParseErr(parser,"object start")
  parser.next
  while true:
    case parser.kind:
...
proc dump*(self: WebhookConversion, s: JsonWriter) =
  s.objectStart()
...
proc isEmpty*(self: CustomResourceConversion): bool =
  ...
  if not self.`webhook`.isEmpty: return false
  true
...
proc watch*(client: Client, t: typedesc[CustomResourceDefinition], name: string, namespace = "default"): Future[FutureStream[CustomResourceDefinition]] {.async.}=
  return await client.watch("/apis/apiextensions.k8s.io/v1", t, name, namespace, loadCustomResourceDefinition)
